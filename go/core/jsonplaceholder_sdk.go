...
type JsonplaceholderSDK struct {
	Mode     string
...
func (sdk *JsonplaceholderSDK) OptionsMap() map[string]any {
...
func (sdk *JsonplaceholderSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *JsonplaceholderSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *JsonplaceholderSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *JsonplaceholderSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *JsonplaceholderSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *JsonplaceholderSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("JsonplaceholderSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *JsonplaceholderSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *JsonplaceholderSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("JsonplaceholderSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *JsonplaceholderSDK {
	...
	sdk := NewJsonplaceholderSDK(sdkopts)
	sdk.Mode = "test"
