...
func TestTodoDirect(t *testing.T) {
	...
	t.Run("direct-load-todo", func(t *testing.T) {
		setup := todoDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-todo", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func todoDirectSetup(mockres any) *todoDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["JSONPLACEHOLDER_TEST_TODO_ENTID"]; ok {
