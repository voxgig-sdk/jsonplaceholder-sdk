...
import (
	...
	omni "github.com/voxgig-sdk/jsonplaceholder-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JsonplaceholderError); is {
				out := map[string]any{
					"name":    "JsonplaceholderError",
...
func omniCtx(arg any, client *sdk.JsonplaceholderSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JsonplaceholderSDK); is {
			client = live
