...
func TestCommentDirect(t *testing.T) {
	t.Run("direct-list-comment", func(t *testing.T) {
		setup := commentDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-comment", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-comment", func(t *testing.T) {
		setup := commentDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-comment", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type commentDirectSetupResult struct {
...
func commentDirectSetup(mockres any) *commentDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["JSONPLACEHOLDER_TEST_COMMENT_ENTID"]; ok {
		...
		return &commentDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &commentDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
