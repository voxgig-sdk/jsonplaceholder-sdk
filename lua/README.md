# Jsonplaceholder Lua SDK

The Lua SDK for the Jsonplaceholder API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Album(nil):list(nil, nil)
...
```lua
local result, err = client:Album(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
```lua
-- Create
local created, _ = client:Album(nil):create({ name = "Example" }, nil)

-- Update
client:Album(nil):update({ id = created["id"], name = "Example-Renamed" }, nil)

-- Remove
client:Album(nil):remove({ id = created["id"] }, nil)
```
...
local result, err = client:Jsonplaceholder(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── jsonplaceholder_sdk.lua    -- Main SDK module
...
The main module (`jsonplaceholder_sdk`) exports the SDK constructor
