...
describe("TodoDirect", function()
  it("should direct-list-todo", function()
    local setup = todo_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-todo", setup.live and "live" or "unit")
  ...
  it("should direct-load-todo", function()
    local setup = todo_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-todo", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function todo_direct_setup(mockres)
