...
describe("PhotoEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Photo(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Photo(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Photo(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = photo_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "photo." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set JSONPLACEHOLDER_TEST_PHOTO_ENTID JSON to run live")
      return
    end
    ...
    local photo_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local photo_ref01_match = {}

    local photo_ref01_list_result, err = photo_ref01_ent:list(photo_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(photo_ref01_list_result)
    ...
    -- REMOVE
    local photo_ref01_match_rm0 = {
      id = photo_ref01_data["id"],
    }
    local _, err = photo_ref01_ent:remove(photo_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local photo_ref01_match_rt0 = {}

    local photo_ref01_list_rt0_result, err = photo_ref01_ent:list(photo_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(photo_ref01_list_rt0_result)
...
function photo_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/photo/PhotoTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read photo test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("JSONPLACEHOLDER_TEST_PHOTO_ENTID")
