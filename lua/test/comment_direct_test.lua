...
describe("CommentDirect", function()
  it("should direct-list-comment", function()
    local setup = comment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-comment", setup.live and "live" or "unit")
  ...
  it("should direct-load-comment", function()
    local setup = comment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function comment_direct_setup(mockres)
