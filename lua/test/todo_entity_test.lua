...
describe("TodoEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Todo(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Todo(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Todo(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = todo_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "todo." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set JSONPLACEHOLDER_TEST_TODO_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local todo_ref01_ent = client:Todo(nil)
    local todo_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local todo_ref01_match = {
      ["user_id"] = setup.idmap["user01"],
    }

    local todo_ref01_list_result, err = todo_ref01_ent:list(todo_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(todo_ref01_list_result)
    ...
    -- UPDATE
    local todo_ref01_data_up0_up = {
      id = todo_ref01_data["id"],
    }

    local todo_ref01_markdef_up0_name = "title"
    local todo_ref01_markdef_up0_value = "Mark01-todo_ref01_" .. tostring(setup.now)
    ...
    -- REMOVE
    local todo_ref01_match_rm0 = {
      id = todo_ref01_data["id"],
    }
    local _, err = todo_ref01_ent:remove(todo_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local todo_ref01_match_rt0 = {
      ["user_id"] = setup.idmap["user01"],
    }

    local todo_ref01_list_rt0_result, err = todo_ref01_ent:list(todo_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(todo_ref01_list_rt0_result)
...
function todo_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/todo/TodoTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read todo test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("JSONPLACEHOLDER_TEST_TODO_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["JSONPLACEHOLDER_TEST_TODO_ENTID"])
