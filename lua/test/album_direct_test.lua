...
describe("AlbumDirect", function()
  it("should direct-list-album", function()
    local setup = album_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-album", setup.live and "live" or "unit")
  ...
  it("should direct-load-album", function()
    local setup = album_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-album", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function album_direct_setup(mockres)
