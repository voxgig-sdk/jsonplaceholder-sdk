...
local AlbumEntity = {}
AlbumEntity.__index = AlbumEntity


function AlbumEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AlbumEntity)
  self._name = "album"
  self._client = client
...
function AlbumEntity:get_name()
  return self._name
end


function AlbumEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AlbumEntity.new(self._client, opts)
end
...
function AlbumEntity:mark_deleted()
  self._deleted = true
end


function AlbumEntity:deleted()
  return true == self._deleted
end


function AlbumEntity:data_set(args)
  if args ~= nil then
...
function AlbumEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AlbumEntity:match_set(args)
  if args ~= nil then
...
function AlbumEntity:match_get()
...
function AlbumEntity:stream(action, args, callopts)
...
function AlbumEntity:_run_op(ctx, post_done)
  local utility = self._utility
