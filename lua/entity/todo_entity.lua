...
local TodoEntity = {}
TodoEntity.__index = TodoEntity


function TodoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TodoEntity)
  self._name = "todo"
  self._client = client
...
function TodoEntity:get_name()
  return self._name
end


function TodoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TodoEntity.new(self._client, opts)
end
...
function TodoEntity:mark_deleted()
  self._deleted = true
end


function TodoEntity:deleted()
  return true == self._deleted
end


function TodoEntity:data_set(args)
  if args ~= nil then
...
function TodoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TodoEntity:match_set(args)
  if args ~= nil then
...
function TodoEntity:stream(action, args, callopts)
...
function TodoEntity:_run_op(ctx, post_done)
  local utility = self._utility
