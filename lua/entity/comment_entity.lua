...
local CommentEntity = {}
CommentEntity.__index = CommentEntity


function CommentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CommentEntity)
  self._name = "comment"
  self._client = client
...
function CommentEntity:get_name()
  return self._name
end


function CommentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CommentEntity.new(self._client, opts)
end
...
function CommentEntity:mark_deleted()
  self._deleted = true
end


function CommentEntity:deleted()
  return true == self._deleted
end


function CommentEntity:data_set(args)
  if args ~= nil then
...
function CommentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CommentEntity:match_set(args)
  if args ~= nil then
...
function CommentEntity:match_get()
...
function CommentEntity:stream(action, args, callopts)
...
function CommentEntity:_run_op(ctx, post_done)
  local utility = self._utility
