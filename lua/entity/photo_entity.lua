...
local PhotoEntity = {}
PhotoEntity.__index = PhotoEntity


function PhotoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PhotoEntity)
  self._name = "photo"
  self._client = client
...
function PhotoEntity:get_name()
  return self._name
end


function PhotoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PhotoEntity.new(self._client, opts)
end
...
function PhotoEntity:mark_deleted()
  self._deleted = true
end


function PhotoEntity:deleted()
  return true == self._deleted
end


function PhotoEntity:data_set(args)
  if args ~= nil then
...
function PhotoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PhotoEntity:match_set(args)
  if args ~= nil then
...
function PhotoEntity:match_get()
...
function PhotoEntity:stream(action, args, callopts)
...
function PhotoEntity:_run_op(ctx, post_done)
  local utility = self._utility
