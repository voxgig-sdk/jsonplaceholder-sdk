...
local JsonplaceholderSDK = {}
JsonplaceholderSDK.__index = JsonplaceholderSDK
...
JsonplaceholderSDK._make_feature = _make_feature


function JsonplaceholderSDK.new(options)
  local self = setmetatable({}, JsonplaceholderSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function JsonplaceholderSDK:options_map()
...
function JsonplaceholderSDK:get_utility()
  return Utility.copy(self._utility)
end


function JsonplaceholderSDK:get_root_ctx()
  return self._rootctx
end


function JsonplaceholderSDK:prepare(fetchargs)
...
function JsonplaceholderSDK:direct(fetchargs)
...
function JsonplaceholderSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function JsonplaceholderSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "JsonplaceholderSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function JsonplaceholderSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function JsonplaceholderSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "JsonplaceholderSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function JsonplaceholderSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = JsonplaceholderSDK.new(sdkopts)
  sdk.mode = "test"
