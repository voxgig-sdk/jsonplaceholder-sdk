...
class CommentEntity
  ...
  def make
    opts = @_entopts.dup
    CommentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JsonplaceholderHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = JsonplaceholderHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Comment.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Comment, Hash] the loaded Comment; raises JsonplaceholderError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JsonplaceholderHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [CommentListMatch, Hash, nil] match filter (any subset of
  #   Comment fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Comment>, Array] the matching Comment items; raises JsonplaceholderError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Comment, Hash] the created Comment; raises JsonplaceholderError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Comment, Hash] the updated Comment; raises JsonplaceholderError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Comment, Hash] the removed Comment; raises JsonplaceholderError on failure
