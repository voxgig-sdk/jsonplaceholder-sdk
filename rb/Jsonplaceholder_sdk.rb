...
require_relative 'Jsonplaceholder_types'
...
class JsonplaceholderSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = JsonplaceholderHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = JsonplaceholderHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, JsonplaceholderFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    JsonplaceholderUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = JsonplaceholderHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = JsonplaceholderHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = JsonplaceholderHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue JsonplaceholderError => err
    ...
    if fetched.is_a?(Hash)
      status = JsonplaceholderHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = JsonplaceholderError.new(
        "graphql_error", "JsonplaceholderSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = JsonplaceholderSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
