# Jsonplaceholder SDK test runner
...
class JsonplaceholderTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JsonplaceholderTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JsonplaceholderTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JsonplaceholderTestRunner.getenv("JSONPLACEHOLDER_TEST_LIVE")
        override = JsonplaceholderTestRunner.getenv("JSONPLACEHOLDER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JsonplaceholderTestRunner.getenv(key)
        ...
        explain = JsonplaceholderTestRunner.getenv("JSONPLACEHOLDER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["JSONPLACEHOLDER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if JsonplaceholderTestRunner._test_control is not None:
            return JsonplaceholderTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JsonplaceholderTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JsonplaceholderTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JsonplaceholderTestRunner.load_test_control()
...
def load_env_local():
    JsonplaceholderTestRunner.load_env_local()


def env_override(m):
    return JsonplaceholderTestRunner.env_override(m)


def entity_data(v):
    return JsonplaceholderTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JsonplaceholderTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JsonplaceholderTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JsonplaceholderTestRunner.load_test_control()


def live_client_options():
    return JsonplaceholderTestRunner.live_client_options()


def live_delay_ms():
    return JsonplaceholderTestRunner.live_delay_ms()
