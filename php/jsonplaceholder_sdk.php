...
class JsonplaceholderSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = JsonplaceholderHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = JsonplaceholderHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!JsonplaceholderFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, JsonplaceholderFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return JsonplaceholderUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = JsonplaceholderHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = JsonplaceholderHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = JsonplaceholderHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new JsonplaceholderError($op . "_allow",
                "JsonplaceholderSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = JsonplaceholderHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new JsonplaceholderError("graphql_error",
                "JsonplaceholderSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Album($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_album === null) {
                $this->_album = new AlbumEntity($this, null);
            }
            return $this->_album;
        }
        return new AlbumEntity($this, $data);
    }
    ...
    public function Todo($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_todo === null) {
                $this->_todo = new TodoEntity($this, null);
            }
            return $this->_todo;
        }
        return new TodoEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new JsonplaceholderSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
