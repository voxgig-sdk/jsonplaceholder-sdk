...
class PostEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set JSONPLACEHOLDER_TEST_POST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function post_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["post01", "post02", "post03", "user01", "user02", "user03"] as $k) {
    ...
    $entid_env_raw = getenv("JSONPLACEHOLDER_TEST_POST_ENTID");
