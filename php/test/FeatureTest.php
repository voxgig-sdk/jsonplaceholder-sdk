...
// Jsonplaceholder SDK feature test
//
...
class FtCtrl extends JsonplaceholderControl
{
...
class FtClient
{
    ...
    private JsonplaceholderUtility $utility;

    public function __construct(JsonplaceholderUtility $utility, array $options)
    {
    ...
    public function get_utility(): JsonplaceholderUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JsonplaceholderContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JsonplaceholderContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JsonplaceholderContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JsonplaceholderConfig::make_feature($name);
    ...
    private static function buildUrl(JsonplaceholderSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JsonplaceholderError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JsonplaceholderResponse($fetched);
        }
    ...
    private function fail(JsonplaceholderContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JsonplaceholderContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
