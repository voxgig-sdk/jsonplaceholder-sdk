...
// Jsonplaceholder SDK retry feature
...
class JsonplaceholderRetryFeature extends JsonplaceholderBaseFeature
{
    ...
    public function init(JsonplaceholderContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (JsonplaceholderContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(JsonplaceholderContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(JsonplaceholderContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
