...
class JsonplaceholderMakeRequest
{
    public static function call(JsonplaceholderContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new JsonplaceholderResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new JsonplaceholderResponse($fetched);
        } else {
