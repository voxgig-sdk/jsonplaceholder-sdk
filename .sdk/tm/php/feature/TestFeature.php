...
// Jsonplaceholder SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class JsonplaceholderTestFeature extends JsonplaceholderBaseFeature
{
    ...
    public function init(JsonplaceholderContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (JsonplaceholderContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = JsonplaceholderParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (JsonplaceholderContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(JsonplaceholderContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = JsonplaceholderParam::call($ctx, $k);
