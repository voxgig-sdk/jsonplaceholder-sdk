...
// Jsonplaceholder SDK pipeline test
//
...
{
    private static function utility(): JsonplaceholderUtility
    {
        return new JsonplaceholderUtility();
    }
    ...
    private static function ctx(array $over = []): JsonplaceholderContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new JsonplaceholderContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new JsonplaceholderOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof JsonplaceholderError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new JsonplaceholderOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = JsonplaceholderMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new JsonplaceholderOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = JsonplaceholderMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new JsonplaceholderOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = JsonplaceholderMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JsonplaceholderSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = JsonplaceholderMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new JsonplaceholderResponse([]),
            'result' => new JsonplaceholderResult([]),
        ]);
        [, $err] = JsonplaceholderMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new JsonplaceholderSpec([]),
            'response' => null,
            'result' => new JsonplaceholderResult([]),
        ]);
        [, $err] = JsonplaceholderMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new JsonplaceholderSpec(['step' => 's']),
            'response' => new JsonplaceholderResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new JsonplaceholderResult([]),
        ]);
        [, $err] = JsonplaceholderMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new JsonplaceholderSpec(['step' => 's']),
            'response' => new JsonplaceholderResponse(self::resp(200, ['v' => 1])),
            'result' => new JsonplaceholderResult([]),
        ]);
        [, $err] = JsonplaceholderMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new JsonplaceholderSpec(['step' => 's']),
            'response' => new JsonplaceholderResponse(self::resp(200, ['v' => 2])),
            'result' => new JsonplaceholderResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        JsonplaceholderMakeResponse::call($ctx);
    ...
    {
        $preset = new JsonplaceholderResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new JsonplaceholderSpec([]),
            'response' => new JsonplaceholderResponse([]),
            'result' => new JsonplaceholderResult([]),
        ...
        [$got, $err] = JsonplaceholderMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new JsonplaceholderResult([])]);
        [, $err] = JsonplaceholderMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new JsonplaceholderSpec([]), 'result' => null]);
        [, $err] = JsonplaceholderMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JsonplaceholderSpec(['step' => 's']),
            'result' => new JsonplaceholderResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new JsonplaceholderOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JsonplaceholderMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JsonplaceholderSpec(['step' => 's']),
            'result' => new JsonplaceholderResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new JsonplaceholderOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JsonplaceholderMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JsonplaceholderResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new JsonplaceholderSpec([]),
            'result' => new JsonplaceholderResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = JsonplaceholderMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JsonplaceholderMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new JsonplaceholderError('boom', 'boom');
        $utility->fetcher = function (JsonplaceholderContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JsonplaceholderSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = JsonplaceholderMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (JsonplaceholderContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = JsonplaceholderMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(JsonplaceholderResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        JsonplaceholderMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (JsonplaceholderContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JsonplaceholderSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = JsonplaceholderMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new JsonplaceholderResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new JsonplaceholderSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = JsonplaceholderMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = JsonplaceholderMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = JsonplaceholderMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new JsonplaceholderResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, JsonplaceholderDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new JsonplaceholderResult(['ok' => false])]);
        $this->expectException(JsonplaceholderError::class);
        JsonplaceholderDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new JsonplaceholderResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', JsonplaceholderMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        JsonplaceholderMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new JsonplaceholderBaseFeature();
        $b = new JsonplaceholderBaseFeature();
        JsonplaceholderFeatureAdd::call($ctx, $a);
        JsonplaceholderFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): JsonplaceholderBaseFeature
    {
        $f = new JsonplaceholderBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        JsonplaceholderFeatureAdd::call($ctx, self::named_feature('a'));
        JsonplaceholderFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        JsonplaceholderFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        JsonplaceholderFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        JsonplaceholderFeatureAdd::call($ctx, $replace);
        ...
        JsonplaceholderFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): JsonplaceholderContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new JsonplaceholderSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JsonplaceholderPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        JsonplaceholderPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new JsonplaceholderResponse(['headers' => null]),
            'result' => new JsonplaceholderResult([]),
        ]);
        JsonplaceholderResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        JsonplaceholderResultBody::call($ctx);
