...
//     (\`new JsonplaceholderSDK(...)\` / \`JsonplaceholderSDK.test(...)\` become
//     \`JsonplaceholderSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'JsonplaceholderSDK'
...
const TEST_SEED = {"entity":{"album":{"test01":{"id":"test01"}},"comment":{"test01":{"id":"test01"}},"photo":{"test01":{"id":"test01"}},"post":{"test01":{"id":"test01"}},"todo":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new JsonplaceholderSDK(...)\` and any
// \`JsonplaceholderSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JsonplaceholderSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JsonplaceholderSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
