...
describe('TodoDirect', async () => {
  ...
  test('direct-load-todo', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-todo', setup.live)) return
  ...
  test('direct-list-todo', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-todo', setup.live)) return
    const { client, calls } = setup
