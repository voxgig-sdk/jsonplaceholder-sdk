...
describe('PhotoDirect', async () => {
  ...
  test('direct-load-photo', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-photo', setup.live)) return
  ...
  test('direct-list-photo', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-photo', setup.live)) return
    if (skipIfMissingIds(t, setup, ["album01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['JSONPLACEHOLDER_TEST_PHOTO_ENTID']
