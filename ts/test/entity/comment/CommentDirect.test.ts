...
describe('CommentDirect', async () => {
  ...
  test('direct-load-comment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-comment', setup.live)) return
  ...
  test('direct-list-comment', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-comment', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['JSONPLACEHOLDER_TEST_COMMENT_ENTID']
