// Command Jsonplaceholder-cli is a CLI + REPL for the Jsonplaceholder SDK,
...
const prompt = "jsonplaceholder"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "album comment photo post todo user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      JSONPLACEHOLDER_APIKEY (api key), JSONPLACEHOLDER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: JSONPLACEHOLDER_APIKEY carries the API key and
	// JSONPLACEHOLDER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("JSONPLACEHOLDER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("JSONPLACEHOLDER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewJsonplaceholderSDK(opts)
