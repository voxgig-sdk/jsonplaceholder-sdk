...
boru-driven command-line client **and** interactive REPL for the Jsonplaceholder
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jsonplaceholder-cli)
make build

# 2. See usage (words, entities, env vars)
./jsonplaceholder-cli --help
...
./jsonplaceholder-cli load 1 album            # {id:1} shorthand
./jsonplaceholder-cli load '{id:1}' album       # explicit match map
./jsonplaceholder-cli update '{name:"x"}' album
./jsonplaceholder-cli list comment

# 5. Override the API base URL for a single call
JSONPLACEHOLDER_BASE=https://api.example.com ./jsonplaceholder-cli list album

# 6. No arguments -> interactive REPL
./jsonplaceholder-cli
jsonplaceholder> list album
jsonplaceholder> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jsonplaceholder-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/jsonplaceholder-cli list album
   ```
...
```sh
./jsonplaceholder-cli load 1 album          # scalar shorthand for {id:1}
./jsonplaceholder-cli load '{id:1}' album     # explicit match map
```
...
```sh
./jsonplaceholder-cli update '{id:1,name:"new"}' album
```
...
Run with no arguments to open a REPL (prompt `jsonplaceholder>`). Each line is
evaluated as its own boru expression:

```text
$ ./jsonplaceholder-cli
jsonplaceholder> list album
jsonplaceholder> /help
jsonplaceholder> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `album`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/jsonplaceholder-cli`. |
...
(`./jsonplaceholder-cli <expr>`) and interactively (the REPL), and expressions compose the
