...
[MCP](https://modelcontextprotocol.io) server exposing the Jsonplaceholder SDK as
two agent tools — `jsonplaceholder_list` and `jsonplaceholder_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jsonplaceholder-mcp)
make build

# 2. Provide credentials via the environment
export JSONPLACEHOLDER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user jsonplaceholder \
  -- /absolute/path/to/jsonplaceholder-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./jsonplaceholder-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// jsonplaceholder_list: first page of records
{ "entity": "album" }
{ "entity": "album", "query": { } }

// jsonplaceholder_load: one record by id
{ "entity": "album", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jsonplaceholder-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user jsonplaceholder \
     -- "$PWD"/dist/*/jsonplaceholder-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `jsonplaceholder_list` and `jsonplaceholder_load` tools now appear
   in new sessions. Ask the agent to *"list album using jsonplaceholder"*
   and it calls `jsonplaceholder_list` with `{"entity":"album"}`.
...
```sh
export JSONPLACEHOLDER_APIKEY=sk_live_xxx            # API key
export JSONPLACEHOLDER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `jsonplaceholder_list` tool
...
### Call the `jsonplaceholder_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `jsonplaceholder_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `jsonplaceholder_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `JSONPLACEHOLDER_APIKEY` | API key sent with every request. |
| `JSONPLACEHOLDER_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

album | comment | photo | post | todo | user

### Smoke test via HTTP (raw JSON-RPC)

```sh
./jsonplaceholder-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"jsonplaceholder_load","arguments":{"entity":"album","query":{"id":1}}}}'
```
